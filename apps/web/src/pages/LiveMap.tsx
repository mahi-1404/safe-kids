      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', fontSize: 14 }}>
            {activeChild ? `${activeChild.name}'s real-time location` : 'No child paired'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={fetchLocation} disabled={loading} style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {location ? (
            <>
              ...
              <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 4 }}>Location Found</div>
            ...
            <>
              <WifiOff size={48} color="#334155" />
              <div style={{ fontWeight: 600, fontSize: 16, color: '#475569' }}>No Location Data</div>
              <div style={{ color: '#334155', fontSize: 13 }}>
                {activeChild ? 'Waiting for child device to report location...' : 'No child device paired'}
              </div>
            </>
          )}
        </div>
        ...
        {/* Bottom bar */}
        {location && (
          <div style={{ padding: '12px 20px', borderTop: '1px solid #334155', display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e' }} />
            <span style={{ fontSize: 13, color: '#94a3b8' }}>
              Last updated: {timeAgo(location.createdAt)}
            </span>
          </div>
        )}
      </div>
